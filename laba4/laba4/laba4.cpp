...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        ...
        case ID_LIST3:
            ...
            {
                int i = SendMessage(hComboBoxGroups, CB_GETCURSEL, 0, 0);
                SendMessage(hComboBoxGroups, CB_GETLBTEXT, i, (LPARAM)group);
            }
            break;
        case ID_LIST1:
            if (HIWORD(wParam) == CBN_SELENDOK)
            {
                int i = SendMessage(hComboBoxNames, CB_GETCURSEL, 0, 0);
                SendMessage(hComboBoxNames, CB_GETLBTEXT, i, (LPARAM)name);
            }
            break;
        case ID_LIST2:
            if (HIWORD(wParam) == CBN_SELENDOK)
            {
                int i = SendMessage(hComboBox2Names, CB_GETCURSEL, 0, 0);
                SendMessage(hComboBox2Names, CB_GETLBTEXT, i, (LPARAM)name2);
            }
            break;
        case ID_BUTTON:
